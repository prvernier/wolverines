...
ui = dashboardPage(
  ...
  dashboardSidebar(
    ...
    selectInput("inv", label = "Feature type:", choices = names(factors)[3:22], 
...
server <- function(input, output) {
  ...
  output$map1 <- renderTmap({
    ...
    if (input$clustButton) {
      ...
      m <- tm_shape(factors) + tm_fill(input$inv, palette = pal, 
                                       style = input$style,
                                       alpha = 1,
                                       colorNA = NULL, 
                                       group = input$inv,
    ...
    if (input$goButton) {
      m <- m + tm_shape(n2()) + tm_borders(col = 'green', lwd = 2, 
                                           group = 'simp.random', zindex = 600) +
               tm_shape(n2()) + tm_borders(col = 'yellow', lwd = 2, 
